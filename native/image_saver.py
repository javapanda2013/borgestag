...
def handle_save_image(url, save_path, skip_thumb=False):
    ...
    v1.41.8:skip_thumb=True の場合は Pillow サムネ生成を skip する。
    ...
    try:
        ...
        # v1.41.8:modal が thumbDataUrl 提供済の通常画像（GIF 以外）は Pillow スキップ
...
def handle_save_image_base64(data_url, save_path, skip_thumb=False):
    ...
    v1.41.8:skip_thumb=True で Pillow サムネ生成を skip。
    """
    try:
    ...
    # v1.41.8:modal 由来 dataUrl を持つ通常画像は Pillow スキップ
    if skip_thumb and not save_path.lower().endswith(".gif"):
        return {"ok": True, "savedPath": final_path}
...
def _dispatch_command(message):
    ...
    if cmd == "LIST_DIR":
        ...
        return handle_save_image(
            ...
            skip_thumb=bool(message.get("skipThumb", False)),  # v1.41.8
        )
