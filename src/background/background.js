...
// ----------------------------------------------------------------
// v1.31.4 で _pendingModal に imageUrl (10MB dataURL) + associatedAudio
...
async function handleAsyncMessage(message, sender) {
  switch (message.type) {
    ...
    // v1.41.5:fire-and-forget 経路。modal は即時最小化、結果は OS 通知＋ runtime.sendMessage で返却
...
// @spec 設計書類/共通/C_Native_Messaging_protocol.md
function sendNative(payload) {
  return new Promise((resolve, reject) => {
    ...
    // v1.30.6 の WeakRef 診断で payload_alive=true × 7 と判明。
...
async function handleSave(payload) {
  ...
  try {
    ...
    // v1.41.7:lastSaveDir set は addSaveHistory の最終集約 set にマージ（個別 set 廃止）
    ...
    // v1.41.8:modal 由来は thumbBlob で直送、Native 由来は thumbDataUrl で受ける
    ...
    return await _withSaveStorageMutex(async () => {
      // v1.41.7:globalTags / recentTags / tagDestinations の個別 set を廃止し、
...
// タグ永続化
// ----------------------------------------------------------------
// v1.41.6:saveTagRecord を廃止。
// 旧 saveTagRecord は tagRecords（key=imageUrl 由来、value={imageUrl, filename, tags, savedAt}）を
// write-only な監査記録として storage.local に蓄積していたが、拡張機能 UI に表示／検索／統計する経路は
...
async function handleInstantSave(imageUrl, pageUrl) {
  try {
    ...
    // v1.46.11 GROUP-69：storage R-M-W を mutex で直列化（tight mutex）
    return await _withSaveStorageMutex(async () => {
      // v1.41.7:lastSaveDir / globalTags / recentTags / recentSubTags / continuousSession の
...
// 既存インストール環境では初回起動時に onupgradeneeded が発火し、既存 thumbnails / externalImportThumbs
...
// 時間（数秒オーダー）に渡るため、「保存履歴は残るが関連付けが失敗する」
// 症状の原因。
...
async function handleSaveMulti(payload) {
  ...
  for (const rawPath of savePaths) {
    ...
    try {
      ...
      // v1.41.7:lastSaveDir / globalTags / tagDestinations の個別 set を廃止し、
  ...
  if (successPaths.length > 0) {
    // v1.41.8:modal 由来は thumbBlob 直送、Native 由来は pyThumbData.dataUrl
    const effectiveThumbBlob    = thumbBlob || null;
    ...
    // v1.46.11 GROUP-69：storage R-M-W を mutex で直列化（tight mutex）
    await _withSaveStorageMutex(async () => {
      // v1.41.7:lastSaveDir / globalTags / recentTags / tagDestinations を集約して 1 回 set
      const _tagStored = await browser.storage.local.get(["globalTags", "recentTags", "tagDestinations"]);
...
// データ量が大きいケースで 145MB の JSON + 67MB の TextEncoder 割当（Profiler 実測）に
...
// ============================================================================
// v1.41.7:保存経路の storage.local.set を 1 回に集約
// ============================================================================
...
async function addSaveHistoryMulti({ imageUrl, filename, savePaths, tags, authors, pageUrl, thumbBlob, thumbDataUrl, thumbWidth, thumbHeight, sessionId, sessionIndex, audioFilename, audioMimeType, audioDurationSec, _extraStorage }) {
  ...
  // v1.41.8:modal が thumbBlob を直送した場合は atob を skip して直接 IDB 保存（btoa/atob 削減）
  let thumbId = null;
